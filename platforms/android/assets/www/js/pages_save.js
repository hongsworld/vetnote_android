...
var user_token ="jq5m8ULGJd-vtG8UKmfB9g"
//임시 토큰 
        ...
        success: function(data) {
          if (data.error_code ==1)  {
            ...
            if (data.wait == true) {
              ...
              alert("sec")
              $("#student_comment_view").text(data.student_comment)
              $("#professor_comment_view").text(data.professor_comment)
              $("#comment_loglist").html("")
              //로그북 없을때
              if (data.comment_logtitle==""){

                $("#comment_loglist").append("" + 
                  "<div class = 'calender_box' style='margin-top:10px; margin-bottom:20px;'> " +
                  "<div class = 'outer'>" +
                      "<div class = 'middle'>" +
                        "<div class = 'inner'>" +
                  ...
                  if (data.comment_logmemo==""){
                    ...
                    $("#comment_loglist").append("" +
                      ...
                      "<div class = 'calender_box' style='margin-top:0px; border-top:0px;min-height:90px; height:auto;'>" +
                        "<div class='log_img' style='position:relative; width:100%; height:200px;background-image:url(" + image_url + "); background-position:center; background-size:100%; background-position:center;background-repeat:no-repeat;'>" +
                        "</div>" +
    ...
    tappable("#submit_comment", {
      onTap: function(e, target){
        ...
        $.ajax({
          ...
          data: {
            selectedMajorId: window.localStorage.getItem("selectedMajorId"),
            student_comment: $("#textarea_student_comment").val(),
            token: user_token 
          },
          success: function(data) {
            if (data.error_code ==1) {
